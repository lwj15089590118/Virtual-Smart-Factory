# -*- coding: utf-8 -*-
"""
vision/measure_model.py —— 多特征尺寸测量仿真模型（阶段3新增）
================================================================
职责：
    把阶段1的"单尺寸高斯抽样"升级为贴近真实视觉测量的【多特征向量】观测过程：
        f0 尺寸偏差mm  ：关键尺寸相对名义值的偏差（带测量噪声）
        f1 圆度um      ：形位公差特征（装配错位类缺陷会显著恶化）
        f2 表面得分    ：表面反射成像质量评分0~100（划痕类缺陷显著降低）
        f3 边缘锐度    :边缘提取清晰度评分0~100（崩缺类缺陷显著降低）

    【真值口径】（阶段3定义，与阶段1理论NG率≈4.6%无缝衔接）：
        真NG = 尺寸超差（过程散布 |d0|>公差，理论≈4.6%，继承阶段1物理模型）
...
class MeasureSimulator:
    ...
    def __init__(self, rng: np.random.Generator,
        ...
        :param sigma:   加工过程散布 σ mm（与阶段1同款口径）
        :param tol:     公差带 ±mm
        """
        self.rng = rng
