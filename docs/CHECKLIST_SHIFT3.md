# 班次3交付清单 —— 视觉算法 + MES 制造执行 + EMS 能源/健康管理

> 本清单对应班次3全部交付物。所有指标均为**仿真验证值**（Python 3.12 @ Windows 10，
...
| `selftest.py` | 新增 C 组用例 C1/C2/C3；报告头更新为班次3 |
...
|---|---|---|---|---|---|
| 班次1规则法(单尺寸) | 97.45% | 93.02% | 64.00% | 75.83% | 36.00% |
| **逻辑回归(本班次主模型)** | **98.95%** | **98.15%** | **84.80%** | **90.99%** | **15.20%** |
| 单类马氏距离(Hotelling T²) | 96.00% | 81.69% | 46.40% | 59.18% | 53.60% |

- 真值口径：NG = 尺寸超差(≈4.6%，继承班次1理论值) ∪ 公差带内隐性缺陷(≈2.5%) → 综合 ≈7%
...
## 五、回归对照基线（班次3实测 vs 班次2基线，均为仿真验证值）
...
> 长跑样本少时逐件波动属正常。需要复现班次1规则法口径时加 `--rule-vision`。
